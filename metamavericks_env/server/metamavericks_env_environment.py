...
class MetamavericksEnvironment(Environment):
    ...
    def reset(self, options: Dict[str, Any] = None) -> MetamavericksObservation:
        ...
        task = (options or {}).get("task", "easy")
        
        # Restore original initialization order and positions
        self.agent_positions = np.array([
            [250.0, 0.0], # Agent 0: Drone
            [200.0, 0.0], # Agent 1: Boat 1
            [300.0, 0.0]  # Agent 2: Boat 2
        ])
        self.agent_velocities = np.zeros((3, 2))
        self.agent_status = ['active'] * 3
        ...
        if task == "easy":
            ...
            self.ocean_current = np.random.uniform(-0.5, 0.5, size=2)
            ...
            self.ocean_current = np.random.uniform(-2.0, 2.0, size=2)
            self.hazard_radius = 60.0
    ...
    def step(self, action: MetamavericksAction) -> MetamavericksObservation:
        ...
        for pos in self.agent_positions:
            if pos[0] <= 5.0 or pos[0] >= self.grid_size - 5.0 or \
               pos[1] <= 5.0 or pos[1] >= self.grid_size - 5.0:
        ...
        # Raw reward for RL logic
        raw_reward = float(step_reward + hazard_penalty + boundary_penalty + success_bonus)
        
        # Scaled reward for submission bot strictly within (0, 1)
        final_reward = (raw_reward + 10.0) / 25.0
    ...
    def _get_observation(self) -> MetamavericksObservation:
        ...
        features = []
        for i in range(3):
            # FIXED MECHANISM: Boats target the Drone, Drone targets the Survivor
            if i == 0:
                target_pos = self.survivor_pos
            else:
                target_pos = self.agent_positions[0] # Target the Drone
                
            rel_pos = target_pos - self.agent_positions[i]
